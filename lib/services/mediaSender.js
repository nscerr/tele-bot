...
function buildCaption(mediaData, platform) {
    ...
    if (mediaData.description) {
        ...
        const truncatedDesc = decodedDesc.length > 300
            ...
            : decodedDesc;
        caption += `📝 <b>Deskripsi:</b> ${escapeHtml(truncatedDesc)}`;
    } else {
        caption += `ℹ️ Konten ${platformLabel}`;
    }

    if (mediaData.author) {
        caption += `\n\n👤 <b>Author:</b> ${escapeHtml(mediaData.author)}`;
    }
...
async function sendSlideMedia(chatId, originalData, uploadedUrls, caption, mediaId, logContext) {
    ...
    for (let i = 0; i < Math.min(images.length, 10); i++) {
        ...
        if (i === 0) {
            mediaObj.caption = `${caption}\n\n<i>(Gambar ${i + 1} dari ${images.length})</i>`;
    ...
    if (sent && images.length > MAX_SLIDE_IMAGE_BUTTONS && replyMarkup) {
        await sendMessage(chatId, `<i>(Info: Hanya tombol unduhan untuk ${MAX_SLIDE_IMAGE_BUTTONS} gambar pertama yang ditampilkan.)</i>`);
    }
}
