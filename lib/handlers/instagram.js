...
const API_ENDPOINTS = [
    ...
    },
    {
        ...
        queryParamName: 'url',
        isSuccess: (responseData) => responseData && responseData.status === 200 && responseData.result && responseData.result.response && responseData.result.response.status === true && responseData.result.response.data && Array.isArray(responseData.result.response.data),
        getData: (responseData) => responseData.result.response,
        getErrorMessage: (responseData) => (responseData && responseData.result && responseData.result.response && responseData.result.response.message) || (responseData && responseData.message) || 'Unknown error or invalid data structure from Vreden API.'
    }
];
...
function processExtractedInstagramData(rawDataFromApi, apiName) {
    ...
    if (apiName === 'Ferdev API') {
        ...
        // Vreden API tidak secara eksplisit memberikan uniqueMediaId seperti shortcode, jadi kita akan generate nanti jika perlu.

        if (rawDataFromApi.data && rawDataFromApi.data.length > 0) {
            rawSlidesData = rawDataFromApi.data.map(item => ({ originalUrl: item.url, type: item.type === 'video' ? 'video' : 'photo', originalThumb: item.thumb }));
...
async function handleInstagramLink(extractedLink, chatId) {
    ...
    for (const api of apisToTry) {
        ...
        try {
            ...
            const apiUrl = `${api.url}?${queryParam}=${encodeURIComponent(extractedLink)}`;
            // MODIFIKASI: console.log -> logger.info
